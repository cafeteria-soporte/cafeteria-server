...
type FindOneDto<E, T> = Omit<FindOneOptions<E>, 'select' | 'relations'> & {
  dto: new () => T;
};
type FindManyDto<E, T> = Omit<FindManyOptions<E>, 'select' | 'relations'> & {
  dto: new () => T;
};
type FindPageDto<E, T> = Omit<
  FindManyOptions<E>,
  'select' | 'relations' | 'skip' | 'take'
...
export class DtoRepository<E extends ObjectLiteral> {
  ...
  async findAndCount<T>({
    dto,
    ...options
  }: FindManyDto<E, T>): Promise<[T[], number]> {
    const { select, relations } = buildFindOptions(dto);
    ...
    return [
      plainToInstance(dto, results, { excludeExtraneousValues: true }),
      total,
    ];
  }
  ...
  async findPaginated<T>({
    ...
    pagination,
    ...options
  }: FindPageDto<E, T>): Promise<PaginationResponseDto<T>> {
