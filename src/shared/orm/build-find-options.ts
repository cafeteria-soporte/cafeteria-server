...
function buildFromMetadata<T>(ctor: new () => T): FindOptions {
  const scalarKeys: string[] =
    Reflect.getMetadata(DTO_SELECT_KEY, ctor.prototype) ?? [];
...
function toSelectOptions(obj: any, seen = new WeakSet()): any {
  ...
  if (Array.isArray(obj))
    return obj.length === 0 ? true : toSelectOptions(obj[0], seen);
  const res: any = {};
...
function toRelationOptions(obj: any, seen = new WeakSet()): any {
  ...
  seen.add(obj);
  if (Array.isArray(obj))
    return obj.length === 0 ? false : toRelationOptions(obj[0], seen);
  const res: any = {};
