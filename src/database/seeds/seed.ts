...
async function seedRoles(): Promise<void> {
    ...
    for (const role of roles) {
        ...
        if (existing.length === 0) {
            ...
            console.log(`  ${YELLOW}⚠  Rol ya existe:${RESET} ${role.name} — saltando.`);
        }
    }
}
...
async function seedRootUser(): Promise<void> {
    ...
    if (existing.length > 0) {
        console.log(`  ${YELLOW}⚠  Usuario root ya existe${RESET} (${username}) — saltando.`);
        return;
    }
    ...
    console.log(`  ${GREEN}✔  Usuario root creado${RESET} → ${BOLD}${username}${RESET}`);
