...
export class ConfirmPaymentUseCase {
  ...
  async execute(
    ...
    const saleTypeId =
      await this.stockMovementTypesService.findIdByName('sale');

    await this.dataSource.transaction(async (manager) => {
      const receiptNumber =
        await this.globalSettingsService.incrementReceiptNumber(manager);
      ...
      await this.userOrdersService.markAsPaid(orderId, receiptNumber, {
        manager,
      });
    });

    await this.userOrdersService.logPaid(
      orderId,
      order.receiptNumber ?? '',
      actingUser,
    );
