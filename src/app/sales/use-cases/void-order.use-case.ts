...
export class VoidOrderUseCase {
    ...
    async execute(orderId: number, dto: VoidOrderDto, actingUser: AuthUser): Promise<UserOrderDto> {
        const order = await this.userOrdersService.findOne(orderId);

        if (order.status === 'voided') {
            throw new BadRequestException({ message: 'Order is already voided.', error: 'ORDER_ALREADY_VOIDED' });
        }
        ...
        const saleVoidTypeId = await this.stockMovementTypesService.findIdByName('sale_void');
        ...
        await this.dataSource.transaction(async (manager) => {
            ...
            await this.userOrdersService.markAsVoided(orderId, dto.reason, actingUser.id, { manager });
        });

        await this.userOrdersService.logVoid(orderId, dto.reason, order.status, actingUser);
