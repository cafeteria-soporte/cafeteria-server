...
export class ProductsService {
  ...
  async findOne<T = ProductDto>(
    id: number,
    options: FindOptions<T> = { dto: ProductDto as any, throwException: true },
  ): Promise<T | null> {
  ...
  async findLowStock(): Promise<ProductDto[]> {
    const entities = await this.rawRepo
    ...
    return Promise.all(
      entities.map((e) =>
        this.repo.findOne({ dto: ProductDto, where: { id: e.id } }),
      ),
