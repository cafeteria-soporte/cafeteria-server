...
export class CategoriesService {
  ...
  ): Promise<FindAllCategoriesResponseDto> {
  ...
  async findOne<T = CategoryDto>(
    id: number,
    options: FindOptions<T> = { dto: CategoryDto as any, throwException: true },
  ): Promise<T | null> {
  ...
  async create(dto: CreateCategoryDto): Promise<CategoryDto> {
    ...
    const category = this.rawRepo.create({
      name: dto.name.trim(),
  ...
  async update(id: number, dto: UpdateCategoryDto): Promise<CategoryDto> {
    ...
    if (dto.name !== undefined) {
      const conflict = await this.rawRepo.findOne({
        where: { name: dto.name },
      });
