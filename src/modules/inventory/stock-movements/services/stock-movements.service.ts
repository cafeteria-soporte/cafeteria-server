...
export class StockMovementsService {
  ...
  ): Promise<PaginationResponseDto<StockMovementDto>> {
  ...
  async findOne<T = StockMovementDto>(
    id: number,
    options: FindOptions<T> = {
      dto: StockMovementDto as any,
      throwException: true,
    },
  ): Promise<T | null> {
  ...
  async create(
    ...
    await this.stockMovementTypesService.findOne(data.movementTypeId, {
      dto: StockMovementTypeDto,
      throwException: true,
    });

    const stockBefore = await this.productsService.findCurrentStock(
      data.productId,
      options,
    );
