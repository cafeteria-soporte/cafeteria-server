...
export class OrderPaymentsService {
    ...
    async findOne(id: number): Promise<OrderPaymentDto> {
        const payment = await this.repo.findOne({ dto: OrderPaymentDto, where: { id } });
    ...
    async create(userOrderId: number, dto: CreateOrderPaymentDto, currentUser: AuthUser): Promise<OrderPaymentDto[]> {
        ...
        await this.auditLog.create({
            ...
            newValue:         JSON.stringify({ paymentMethodId: dto.paymentMethodId, amount }),
        });
