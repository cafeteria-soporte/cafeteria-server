...
import { OrderItemNotFoundException, InsufficientStockException, ProductInactiveException } from '../exceptions';
...
export class OrderItemsService {
    ...
    async create(userOrderId: number, dto: CreateOrderItemDto, currentUser: AuthUser): Promise<OrderItemDto[]> {
        const product = await this.productsService.findOne(dto.productId, { dto: ProductDto, throwException: true }) as ProductDto;

        if (!product.active) throw new ProductInactiveException();

        const existing = await this.repo.findOne({ dto: OrderItemDto, where: { userOrderId, productId: dto.productId } });
        const newQuantity = (existing?.quantity ?? 0) + dto.quantity;

        if (product.currentStock < newQuantity) throw new InsufficientStockException(product.currentStock);

        if (existing) {
            await this.rawRepo.update(
                { userOrderId, productId: dto.productId },
                { quantity: newQuantity, subtotal: newQuantity * Number(product.salePrice) },
            );
        } else {
        ...
        await this.auditLog.create({
            ...
            newValue:         JSON.stringify({ productId: dto.productId, quantity: dto.quantity }),
        });

        return this.findByOrder(userOrderId);
    }

    async remove(userOrderId: number, productId: number, currentUser: AuthUser): Promise<void> {
        const item = await this.repo.findOne({ dto: OrderItemDto, where: { userOrderId, productId } });
