...
export class FindShiftRecordsDto extends PaginationParamsDto {
    ...
    @ApiPropertyOptional({ description: 'Fecha de inicio ISO 8601 (openedAt >= from)', example: '2025-01-01' })
    @IsOptional()
    @IsDateString()
    from?: string;

    @ApiPropertyOptional({ description: 'Fecha de fin ISO 8601 (openedAt <= to)', example: '2025-12-31' })
    @IsOptional()
    @IsDateString()
    to?: string;
}
