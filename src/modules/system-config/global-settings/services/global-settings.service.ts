...
export class GlobalSettingsService {
  ...
  async findOne<T = GlobalSettingDto>(
    key: string,
    options: FindOptions<T> = {
      dto: GlobalSettingDto as any,
      throwException: true,
    },
  ): Promise<T | null> {
  ...
  async findValueByKey(key: string): Promise<string | null> {
    const setting = await this.rawRepo.findOne({
      where: { key },
  ...
  async incrementReceiptNumber(manager: EntityManager): Promise<string> {
    ...
    const prefix = await manager.findOne(GlobalSetting, {
  ...
  async update(
    ...
    await this.rawRepo.update(
      { key },
      { value: dto.value, updatedById: actingUser.id },
    );
