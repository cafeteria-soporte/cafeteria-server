...
export class SalesFilterDto {
    ...
    @ApiPropertyOptional({ enum: GroupBy, default: GroupBy.DAY, description: 'Agrupación temporal para by-period' })
    @IsOptional()
    @IsEnum(GroupBy)
    groupBy?: GroupBy = GroupBy.DAY;

    @ApiPropertyOptional({ example: 10, default: 10, description: 'Límite de resultados para top-products' })
    @IsOptional()
    @Type(() => Number)
    @IsInt()
    @Min(1)
