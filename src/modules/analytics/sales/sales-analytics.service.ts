...
import { ExportFormat, GroupBy, SalesExportFilterDto, SalesFilterDto } from './dto/in/sales-filter.dto';
...
export class SalesAnalyticsService {
    ...
    async findByPeriod(filter: SalesFilterDto): Promise<SalesByPeriodDto[]> {
        const groupBy = VALID_GROUP_BY.includes(filter.groupBy ?? '') ? filter.groupBy : GroupBy.DAY;
    ...
    async findByCategory(filter: SalesFilterDto): Promise<SalesByCategoryDto[]> {
        ...
        return rows.map(r => ({
            ...
            revenue:    Number(r.revenue),
            orderCount: Number(r.order_count),
            percentage: total > 0 ? Math.round((Number(r.revenue) / total) * 1000) / 10 : 0,
        }));
    }
    ...
    private generateCsv(data: SalesByPeriodDto[], filter: SalesFilterDto): StreamableFile {
    ...
    private async generatePdf(
        ...
        const docDef = {
            content: [
                ...
                {
                    columns: [
                        { text: `Ingresos Totales\nBs ${totalRevenue.toFixed(2)}`,   style: 'kpi' },
                        { text: `Órdenes Totales\n${totalOrders}`,                  style: 'kpi' },
                        { text: `Ticket Promedio\nBs ${avgTicket.toFixed(2)}`,      style: 'kpi' },
                    ],
                    margin: [0, 0, 0, 16],
                },
