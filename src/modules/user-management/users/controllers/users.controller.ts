import { Controller, Get, Patch, Param, Query, HttpCode, HttpStatus, ParseIntPipe } from '@nestjs/common';
...
export class UsersController {
    ...
    findOne(@Param('id', ParseIntPipe) id: number) {
        return this.usersService.findOneById(id, { dto: UserDto, throwException: true });
    }
    ...
    @ApiOperation({
        ...
        description: 'Solo se puede desactivar usuarios con rol inferior al propio. No se puede desactivar la propia cuenta.',
    })
    @ApiNoContentResponse({ description: 'Cuenta desactivada.' })
    @ApiNotFound()
    @ApiValidationError()
    deactivate(@Param('id', ParseIntPipe) id: number, @CurrentUser() actor: AuthUser) {
